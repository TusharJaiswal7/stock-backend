...
def get_live_nifty():
    """Fetch live Nifty 50 index value."""
    try:
...
def get_live_price(symbol: str):
    """Fetch live NSE stock price using yfinance."""
    try:
        import yfinance as yf
...
async def today_plan(force: bool = False):
    ...
    nifty_str = f"Current Nifty 50 level: {nifty}" if nifty else "Nifty level unavailable"
...
async def stock_pick(req: StockPickRequest):
    capital = 25000
    live_price = None
    if req.symbol and STOCK_API_PROVIDER == 'yfinance':
        live_price = get_live_price(req.symbol)

    price_info = f"Current live NSE price: Rs{live_price}" if live_price else "Fetch current price from your knowledge"
...
async def portfolio_plan(req: PortfolioPlanRequest):
    ...
    news_str = "\n".join(news) if news else "No news available"
    nifty = get_live_nifty()
    nifty_str = f"Current Nifty: {nifty}" if nifty else ""
